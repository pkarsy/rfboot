...
// diko mou
void CC1101::leavePowerDownState()
{
	// Comming from RX state, we need to enter the IDLE state first
	cmdStrobe(CC1101_SIDLE);
 ...
 *    False otherwise
 */
/* boolean CC1101::sendData(const CCPACKET & packet)
{
	byte marcState;
	bool res = false;
...
bool CC1101::sendPacket(const byte* packet, byte length)
{
...
uint32_t CC1101::sendBurstPacket(const byte* packet, const byte length, const uint32_t msec) {
	byte marcState;
	uint32_t counter=0;
...
byte CC1101::getPacket(byte* packet)
{
...
bool CC1101::print(const __FlashStringHelper* fmt, ...) {
